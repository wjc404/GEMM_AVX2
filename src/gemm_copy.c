...
/* initialization complete, start packing */
  for(bcol=0;bcol<GEMM_LOOP_TIMES_N;bcol++){
    for(bsub1=GEMM_UNROLL_N-1;bsub1>=0;bsub1--){
      for(brow=0;brow<GEMM_LOOP_TIMES_K*8;brow++){
      ...
      if(bsub1==0) for(bsub2=0;bsub2<GEMM_UNROLL_N;bsub2++) inb[bsub2]-=GEMM_BLOCK_DIM_K;
...
static void load_reg_b_r(const FLOAT * __restrict__ bstartpos,FLOAT * __restrict__ bblk,int ldb,const FLOAT * __restrict__ alpha){
  const FLOAT *bin1,*bin2,*bin3,*bin4;FLOAT *bout;int bcol,brow,bsub1,bsub2;const FLOAT ALPHA=*alpha;
  bin1=bstartpos;bin2=bin1+ldb;bin3=bin2+ldb;bin4=bin3+ldb;int bshift=4*ldb-GEMM_BLOCK_DIM_N;
  for(brow=0;brow<GEMM_LOOP_TIMES_K*8;brow+=4){
    bout=bblk+brow*GEMM_UNROLL_N;
    for(bcol=0;bcol<GEMM_LOOP_TIMES_N;bcol++){
      bcopy_4row(GEMM_UNROLL_N)
      bout+=GEMM_UNROLL_N*GEMM_BLOCK_DIM_K;
    }
    bin1+=bshift;bin2+=bshift;bin3+=bshift;bin4+=bshift;
  }
  for(bsub1=GEMM_UNROLL_N-1;bsub1>0;bsub1--){
    for(;brow<GEMM_LOOP_TIMES_K*8*(GEMM_UNROLL_N+1-bsub1);brow+=4){
      bout=bblk+brow*GEMM_UNROLL_N+(GEMM_LOOP_TIMES_N-1)*GEMM_UNROLL_N*GEMM_BLOCK_DIM_K+bsub1;
...
static void load_irreg_b_c(const FLOAT * __restrict__ bstartpos,FLOAT * __restrict__ bblk,int ldb,int ndim,int kdim,const FLOAT * __restrict__ alpha){//dense rearr(old) lazy mode
...
static void load_irreg_b_r(const FLOAT * __restrict__ bstartpos,FLOAT * __restrict__ bblk,int ldb,int ndim,int kdim,const FLOAT * __restrict__ alpha){//dense rearr(old) lazy mode
