...
/* start main calculation here */
    ...
    for(k_count=0;k_count<(*k);k_count+=BLOCKDIM_K){
      ...
      if(NOTRANSB) { sgemm_ncopy_16(b_current_pos,b_buffer,*ldb,k_subdim,*n); b_current_pos += BLOCKDIM_K; }
      else { sgemm_tcopy_16(b_current_pos,b_buffer,*ldb,*n,k_subdim); b_current_pos += (int64_t)(*ldb) * BLOCKDIM_K; }
      ...
      for(m_count=0;m_count<(*m);m_count+=BLOCKDIM_M){
        ...
        if(NOTRANSA) { sgemm_tcopy_6(a_current_pos,a_buffer,*lda,m_subdim,k_subdim); a_current_pos += BLOCKDIM_M; }
        else { sgemm_ncopy_6(a_current_pos,a_buffer,*lda,k_subdim,m_subdim); a_current_pos += (int64_t)(*lda) * BLOCKDIM_M; }
