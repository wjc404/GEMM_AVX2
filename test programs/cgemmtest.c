...
//this program is for tests (about performances and outputs) of user-provided cgemm library against Intel MKL
/*an example of compilation command
gcc -fopenmp cgemmtest.c -Wl,--start-group /opt/intel/mkl/lib/intel64/libmkl_intel_lp64.a /opt/intel/mkl/lib/intel64/libmkl_gnu_thread.a /opt/intel/mkl/lib/intel64/libmkl_core.a -Wl,--end-group -lpthread -lm -ldl -o cgemmtest
*/

int main(int argc, char* argv[]) // command line: ./cgemmtest [niter] [m] [n] [k] [upperbound] [lowerbound] [transa] [transb]
{
// variables blow for dynamic linking
        void (*cgemmroutine1)(char *transa, char *transb, int *m, int *n, int *k, float *alpha, float *a, int *lda, float *b, int *ldb, float *beta, float *c, int *ldc);
        void cgemm(char *transa, char *transb, int *m, int *n, int *k, float *alpha, float *a, int *lda, float *b, int *ldb, float *beta, float *c, int *ldc);
        void *handle1;
        char cgemmpath1[200],cgemmname1[18]; // get linking info from stdin
        char *DLERR;
...
//print test informations
        printf("A SIMPLE CGEMM TESTING PROGRAM\n");
        printf("Matrix elements will be stored in column-major order.\n");
        printf("Transposition: matrix A: %c; matrix B: %c.\n",trsa,trsb);
        printf("CGEMM will be called in this way:\n");
        printf("    <cgemmname>(&transa, &transb, &m, &n, &k, &alpha, A, &lda, B, &ldb, &beta, C, &ldc)\n");
        printf("Please make sure that the cgemm library you provided can respond normally to the call above\n\n");

// load the tested cgemm routine
        dlerror();
        printf("Enter your cgemm library path(path to the *.so file):");
        scanf("%s",cgemmpath1);
        handle1 = dlopen(cgemmpath1,RTLD_LAZY);
        DLERR = dlerror();
        if (DLERR) {  
            printf ("Error locating the library: %s\n",DLERR);  
            exit(1);  
        }
        printf("Enter the function name of cgemm in your library(e.g., cgemm):");
        scanf("%s",cgemmname1);
        cgemmroutine1 = dlsym(handle1,cgemmname1);
        DLERR = dlerror();
        if (DLERR) {
            printf ("Error locating cgemm function in your library: %s\n",DLERR);
            cgemmroutine1=NULL;dlclose(handle1);handle1=NULL;
            exit(1);
        }
        printf("Now test cgemm with small matrices, identical results indicate successful linking\n");
        (*cgemmroutine1)(&temptrsa, &temptrsb, &tempm, &tempn, &tempk, alpha, a, &templda, b, &templdb, beta, d, &templdc);
...
// print test information
	...
	printf("Number of iterations %d \n",niters);
        ...
        printf("Now start CGEMM iterations \n\n");

// **start CGEMM-compare iterations here**
    for (i=0; i<niters; ++i) {
...
// start C = (4+2i)*(AB)1
	alpha[0]=4.0;alpha[1]=2.0;beta[0]=2.0;beta[1]=0.0;
        ...
        cgemm(&trsa, &trsb, &m, &n, &k, alpha, A, &lda, B, &ldb, beta, C, &ldc);
...
// start C = (3+i)C - (10+10i)(AB)2
	alpha[0]=-10.0;alpha[1]=-10.0;beta[0]=3.0;beta[1]=1.0;//beta=3+i;alpha=-(4+2i)(3+i)=-10-10i
        gettimeofday(&starttime,0);
        (*cgemmroutine1)(&trsa, &trsb, &m, &n, &k, alpha, A, &lda, B, &ldb, beta, C, &ldc);
        gettimeofday(&endtime,0);
...
// **end of iterations, print calculated FLOPS of routine 1 and 2 in each iteration**
	...
	printf("FP operation count per cgemm call:%ld\n",ops);
        printf("lib1: %s.\n",cgemmpath1);
