...
#define KERNELkr {\
   ...
   c1=IRREG_VEC_FMADD(a1,b1,c1);\
   a1=IRREG_VEC_LOADA(atemp);atemp+=32/IRREG_SIZE;\
   c2=IRREG_VEC_FMADD(a1,b1,c2);\
   a1=IRREG_VEC_LOADA(atemp);atemp+=32/IRREG_SIZE;\
   c3=IRREG_VEC_FMADD(a1,b1,c3);\
   a1=IRREG_VEC_LOADA(atemp);atemp+=32/IRREG_SIZE;\
   c4=IRREG_VEC_FMADD(a1,b1,c4);\
}
#define KERNELk1 {\
   b1=IRREG_VEC_BROAD(btemp);btemp++;\
   ...
   a1=IRREG_VEC_LOADA(atemp);atemp+=32/IRREG_SIZE;\
   c1=IRREG_VEC_FMADD(a1,b1,c1);c5=IRREG_VEC_FMADD(a1,b2,c5);c9=IRREG_VEC_FMADD(a1,b3,c9);\
   a1=IRREG_VEC_LOADA(atemp);atemp+=32/IRREG_SIZE;\
   c2=IRREG_VEC_FMADD(a1,b1,c2);c6=IRREG_VEC_FMADD(a1,b2,c6);c10=IRREG_VEC_FMADD(a1,b3,c10);\
   a1=IRREG_VEC_LOADA(atemp);atemp+=32/IRREG_SIZE;\
   c3=IRREG_VEC_FMADD(a1,b1,c3);c7=IRREG_VEC_FMADD(a1,b2,c7);c11=IRREG_VEC_FMADD(a1,b3,c11);\
   a1=IRREG_VEC_LOADA(atemp);atemp+=32/IRREG_SIZE;\
   c4=IRREG_VEC_FMADD(a1,b1,c4);c8=IRREG_VEC_FMADD(a1,b2,c8);c12=IRREG_VEC_FMADD(a1,b3,c12);\
}
#define KERNELk2 {\
...
#define STORE_C_1col_multa1(c1,c2,c3,c4,a1) {\
   c1=IRREG_VEC_FMADD(a1,IRREG_VEC_LOADU(ctemp),c1);\
   c2=IRREG_VEC_FMADD(a1,IRREG_VEC_LOADU(ctemp+32/IRREG_SIZE),c2);\
   c3=IRREG_VEC_FMADD(a1,IRREG_VEC_LOADU(ctemp+64/IRREG_SIZE),c3);\
   c4=IRREG_VEC_FMADD(a1,IRREG_VEC_LOADU(ctemp+96/IRREG_SIZE),c4);\
...
#define STORE_C_1col_nomult(c1,c2,c3,c4) {\
...
#define STOREIRREGM_C_1col(c1,c2,c3,c4,a1) {\
   c1=IRREG_VEC_FMADD(a1,IRREG_VEC_MASKLOAD(ctemp,ml1),c1);\
   c2=IRREG_VEC_FMADD(a1,IRREG_VEC_MASKLOAD(ctemp+32/IRREG_SIZE,ml2),c2);\
   c3=IRREG_VEC_FMADD(a1,IRREG_VEC_MASKLOAD(ctemp+64/IRREG_SIZE,ml3),c3);\
   c4=IRREG_VEC_FMADD(a1,IRREG_VEC_MASKLOAD(ctemp+96/IRREG_SIZE,ml4),c4);\
...
static void gemmblkirregkccc(FLOAT * __restrict__ ablk,FLOAT * __restrict__ bblk,FLOAT * __restrict__ cstartpos,int ldc,int kdim,FLOAT * __restrict__ beta){
  ...
  for(ccol=0;ccol<BlkDimN;ccol+=3){//loop over cblk-columns, calculate 3 columns of cblk in each iteration.
   ...
   STORE_C_1col_multa1(c1,c2,c3,c4,a1)
   STORE_C_1col_multa1(c5,c6,c7,c8,a1)
   STORE_C_1col_multa1(c9,c10,c11,c12,a1)
  }
}
static void gemmblkirregnccc(FLOAT * __restrict__ ablk,FLOAT * __restrict__ bblk,FLOAT * __restrict__ cstartpos,int ldc,int ndim){
  ...
  for(ccol=0;ccol<ndim-2;ccol+=3){//loop over cblk-columns, calculate 3 columns of cblk in each iteration.
   ...
   STORE_C_1col_nomult(c1,c2,c3,c4)
   STORE_C_1col_nomult(c5,c6,c7,c8)
   STORE_C_1col_nomult(c9,c10,c11,c12)
  }
  for(;ccol<ndim;ccol++){
   INIT_1col
   atemp=ablk;
...
static void gemmblkirregccc(FLOAT * __restrict__ ablk,FLOAT * __restrict__ bblk,FLOAT * __restrict__ cstartpos,int ldc,int mdim,int ndim,int kdim,FLOAT * __restrict__ beta){
  ...
  ml4=_mm256_setr_epi32(0,-(mdim>12),0,-(mdim>13),0,-(mdim>14),0,-(mdim>15));
#else
  ...
  ml4=_mm256_setr_epi32(-(mdim>24),-(mdim>25),-(mdim>26),-(mdim>27),-(mdim>28),-(mdim>29),-(mdim>30),-(mdim>31));
#endif
  ctemp=cstartpos;btemp=bblk;
  for(ccol=0;ccol<ndim-2;ccol+=3){//loop over cblk-columns, calculate 3 columns of cblk in each iteration.
   cpref=ctemp;
   ...
   for(acol=0;acol<kdim;acol++) KERNELk1//loop over ablk-columns, load 1 column of ablk in each micro-iteration.
   a1=IRREG_VEC_BROAD(beta);
   STOREIRREGM_C_1col(c1,c2,c3,c4,a1)
   STOREIRREGM_C_1col(c5,c6,c7,c8,a1)
   STOREIRREGM_C_1col(c9,c10,c11,c12,a1)
  }
  ...
  for(;ccol<ndim;ccol++){
   ...
   STOREIRREGM_C_1col(c1,c2,c3,c4,c9)
  }
}
